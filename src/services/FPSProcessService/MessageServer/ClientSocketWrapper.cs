...
{
    [Serializable()]
    public abstract class ClientSocketWrapper
    {
        ...
        protected TcpClient _baseSocket = null;
        protected ReceiveState _state = null;
        protected string _curClientIP = string.Empty;
        protected NetworkStream _clientStream = null;


        public ClientProcessorWrapper.DEOFCheck DEof = null;
        public ClientProcessorWrapper.DEOFLengthCheck DEofLength = null;
        public ClientProcessorWrapper.DProcessMsg ProcessFn = null;

        public ClientProcessorWrapper.DEOFCheckS DEofS = null;
        public ClientProcessorWrapper.DEOFLengthCheckS DEofLengthS = null;
        public ClientProcessorWrapper.DProcessMsgS ProcessFnS = null;
        ...
        public TcpClient ConnectedSocket
        {
            get
            {
                return _baseSocket;
        ...
        public void Send(byte[] data)
        {
            ...
            try
            {
                ...
                _clientStream.BeginWrite(data, 0, data.Length, new AsyncCallback(SendCallback), null);
            }
        ...
        protected virtual void SocketReceive()
        {
            try
            {
                if (IsMessageLengthEnabled && _state.MessageLength == 0)
                {
                    ...
                    _clientStream.BeginRead(lengthbuffer, 0, MessageLengthBuffer,  _cb, _baseSocket);
                }
                else if (IsMessageLengthEnabled)
                {
                    ...
                    if (!_bClosing) _clientStream.BeginRead(_state.buffer, 0, tSize, _cb, null);
                }
                else
                {
                    //RaiseLogError("Waiting for data");
                    ...
                    if (!_bClosing) _clientStream.BeginRead(_state.buffer, 0, _state.BufferSize, new AsyncCallback(ReadCallback), null);
                }
            }
        ...
        protected void ReadLengthCallBack(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                bytesRead = _clientStream.EndRead(ar);
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        RaiseLogError("Byte : " + _state.MessageLength.ToString());
                    }
            ...
            if (_settings.SocketMode == ESocketMode.Binary)
            {
                ...
                if (isEOF)
                {
                    ...
                    try
                    {
                        ...
                        //ProcessFn.BeginInvoke(message, this, new AsyncCallback(ProcessFunctionCallback), null);
                    }
                    catch(Exception ex)
                    {
                        RaiseLogError(ex);
        ...
        private void ProcessFunctionCallback(IAsyncResult result)
        {
            ProcessFn.EndInvoke(result);
        }
        ...
        {
            if (_curClientIP == string.Empty && _baseSocket != null && !this.Closing) _curClientIP = _baseSocket.Client.RemoteEndPoint.ToString();
            return _curClientIP;
        }
